...
export default function StudentHomePage() {
  ...
  const [page, setPage] = useState<"home" | number>("home");

  const ch = typeof page === "number" ? CHAPTERS.find((c) => c.id === page) : null;
        ...
        <div style={{ maxWidth: 820, margin: "0 auto", padding: "36px 20px" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <p style={{ color: "#6b7280", fontSize: 14 }}>
              Select a chapter to begin. Your answers and marks are saved to your profile.
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 14 }}>
            {CHAPTERS.map((c) => (
              <ChapterCard key={c.id} chapter={c} onClick={() => setPage(c.id)} />
            ))}
          </div>
        </div>
      ) : ch ? (
